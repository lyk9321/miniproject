...
# 실행 방법: python typing_game_v4.py
#
# [이번 버전에서 바뀐 점]
#  1. Q/T 키가 더 이상 기능키가 아님! → 영어 단어 'quit', 'type' 정상 입력 가능
#  2. ESC 키 하나로 모든 메뉴(리셋/메인메뉴/종료)에 접근
#  3. 게임오버에서 R=같은 언어 재시작, M=메인메뉴, X=프로그램 종료
#  4. 게임오버에서 한/영 입력 상태와 상관없이 조작키가 항상 먹음
#  5. 언어 선택 시 입력 모드 안내 문구 표시
...
def draw_game_over(screen):
    ...
    screen.blit(go_surface, go_surface.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 130)))

    # 최종 점수
    score_text = f"최종 점수 / Final Score: {score}"
    score_surface = font_info.render(score_text, True, TITLE_COLOR)
    screen.blit(score_surface, score_surface.get_rect(center=(WIDTH // 2, HEIGHT // 2 - 50)))

    # 선택 메뉴 (팝업 스타일)
    menu_title = font_popup.render("메뉴", True, TITLE_COLOR)
    screen.blit(menu_title, menu_title.get_rect(center=(WIDTH // 2, HEIGHT // 2 + 10)))
...
def handle_event_game_over(event):
    ...
    IME가 꺼져 있어서 한국어 입력 상태라도 R/M/X가 정상 동작!
    """
    if event.type == pygame.KEYDOWN:
...
def main():
    ...
    font_help = get_font(16)
    font_popup = get_font(26)
    font_announce = get_font(18)
